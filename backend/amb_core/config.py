...
class Settings(BaseSettings):
    ...
    # LLM — provider-agnostic. Pick a provider; the memo layer resolves the matching
    # client, or falls back to the deterministic template when none is configured.
    llm_provider: str = Field("anthropic", alias="AMB_LLM_PROVIDER")  # anthropic | openai | none
    ...
    openai_model: str = Field("gpt-4o-2024-11-20", alias="AMB_OPENAI_MODEL")
    ...
    def has_llm(self) -> bool:
        """True when the selected provider has a usable key (else: template fallback)."""
