...
def render_markdown(memo: Memo) -> str:
    ...
    breaking the former import cycle."""
    out = [f"# {memo.title}", ""]
    ...
    for s in memo.shortlist:
        m = s.metrics
        out.append(
            f"| {s.rank} | {s.name} | {s.strategy} | {_pct(m.get('ann_return'))} | "
            f"{_num(m.get('sharpe'))} | {_num(m.get('sortino'))} | {_num(m.get('calmar'))} | {_pct(m.get('max_drawdown'))} |"
        )
    out.append("")
